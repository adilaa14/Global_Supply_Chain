...
foreach ($activeRoutes as $route) {
    ...
    if ($originPort) $originCode = $originPort->port_code;
    
    $destPort = \App\Models\Port::find($route->destination_port_id);
    if (!$destPort) continue;
    $destCode = $destPort->port_code;

    $pathFromOriginToHub = array_reverse($hubRoutes[$originCode] ?? $hubRoutes['SGSIN']);
    $pathFromHubToDest = $hubRoutes[$destCode] ?? $hubRoutes['SGSIN'];
...
echo "All routes updated with perfectly safe Sunda Strait trunk.\n";
